...
export class UserProfileElement extends App.View {
  ...
  render() {
    const { userid, name, nickname, city = [] } = this.profile;
  ...
  _renderAvatar() {
    const { avatar, name, nickname, color } = (this.profile || {}) as Profile;
...
export class UserProfileEditElement extends UserProfileElement {
      ...
      <section>
        <form @submit=${this._handleSubmit}>
          <dl>
            ...
            <dt>Airports</dt>
            <dd><input name="airports" .value=${airports.join(", ")} /></dd>
          </dl>
  ...
  _handleSubmit(event: Event) {
    ...
    if (this.profile) {
      ...
      if (this.newAvatar) entries.push(["avatar", this.newAvatar]);
